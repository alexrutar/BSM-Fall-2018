...
\begin{proof}
    ...
    The first two inequalities are obvious: we must show that $\lim\sup\int_X|f_f_n|\d{\mu}\leq 0$.
    Firstly, we have
    \begin{align*}
...
\begin{proof}
    Will do later.
    It's pretty fun - it's a construction using the Dyadic rationals.
\end{proof}
...
\begin{definition}
    ...
    % The \textbf{Lebesgue inner measure} is defined $\mu(E)=\sup\{\mu(G):K\subset E\text{ compact}\}$.
\end{definition}
\begin{definition}
    We say that $E\subset X$ is \textbf{regular} if the inner and outer measures coincide.
